...
리뷰 직후 같은 세션에서 손본 것들. 백엔드 344개·프론트 310개 테스트 통과.
...
| BM-04 | `Ctrl+J` 점퍼의 원격 항목은 재접속하면 죽고, 세션마다 중복 누적된다 — frecency 키를 host_alias 로, 점퍼가 alias 로 (필요시 접속 후) 연다. 구버전 항목은 폐기 | 고침 |
| BM-05 | Bookmark vs HostFavorite 두 모델의 분열이 사용자 눈에 보인다 (Phase 3 "통합" 은 렌더링만) — 원격 즐겨찾기에 L/R 배지 추가. 데이터 통합은 후속 | 일부 |
| BM-06 | 북마크에 단축키/번호를 줄 수 없다 — `Ctrl+1..9` / `Alt+1..9` 가 비어 있음 — bookmark.goto1..9 (Alt+Shift+1..9) | 고침 |
| BM-10 | 추가가 조용히 실패·무시되고, 추가 후 어디 생겼는지 피드백이 없다 — 추가·중복·실패 토스트 + 접힌 섹션 자동 펼침 | 고침 |
| CF-01 | `settings.toml.example` 이 존재하지 않는 설정 17개를 약속하고, 경로·구조도 실제와 다름 — 예시 파일을 실제 스키마(평면 16키)로 다시 씀 + 저장 위치 명시 | 고침 |
| CF-02 | 설정 항목 커버리지 — 사용자가 기대하는 핵심 동작에 설정이 없음 — size_units / date_format / dirs_first / editor_command + F4(file.edit) 추가 | 고침 |
| CF-03 | i18n — 설정 화면 4개 섹션이 통째로 영어 하드코딩, 그 외 30여 곳 산재 — 설정 General 섹션·목록 컬럼 헤더·스토어 토스트 i18n. Aliases/ExtIcons/OpenWith 섹션은 후속 | 일부 |
| CF-04 | 글로벌 검색 — 필터 없음, 결과에 아무 작업도 못 함 — 크기·기간·종류 필터 + 결과에 크기·수정시각. 정규식·결과에서 바로 작업은 후속 | 일부 |
| CF-05 | 메뉴·툴팁·치트시트의 키 힌트가 하드코딩 — 리바인딩·플랫폼 미반영, altKeys 미표시 — 메뉴·툴바·경로바·상단바가 useKeyHint 로 현재 키맵을 표시. i18n 문구에서 키 제거 | 고침 |
| CF-06 | 토스트가 DESIGN.md 와 어긋남 — 에러가 6초 후 사라지고, 성공 토스트가 잦으며, 행동할 수 없다 — 에러 토스트가 수동 닫기까지 유지 + 성공 토스트 절제. 액션 버튼(되돌리기/재시도)은 후속 | 일부 |
| CF-07 | "영구 삭제" 가 설정 OFF 여도 메뉴·팔레트에 노출되고 실행 시점에야 실패 — 설정이 꺼져 있으면 메뉴·팔레트에 아예 등록하지 않는다 | 고침 |
| CF-08 | 커맨드 팔레트 — 최근 사용 없음, 카테고리 그룹 없음, 한국어 UI 에서 영어 검색 불가 — 최근 사용 우선 + 번역·원문·id 매칭. 카테고리 그룹 헤더와 모드 접두는 후속 | 일부 |
| CF-13 | 에러 문구 파이프라인 — 원인별 다음 행동 없음, 검색 패널은 enum 이름 노출 — 검색 패널이 formatErr 사용. 원인별 액션 버튼은 후속 | 일부 |
...
| NV-02 | macOS 시스템 단축키와 정면 충돌하는 기본 키맵 (Ctrl→⌘ 일괄 매핑의 부작용) — 맥 전용 기본 키(정렬 ⌘⌥1..5, 숨김 ⇧⌘., 미리보기 ⇧⌘P, 경로 ⇧⌘G) + 숫자열을 e.code 로 | 고침 |
| NV-03 | 상위로 갈 때 떠나온 폴더에 커서가 앉지 않음 + 뒤로/앞으로 시 커서·스크롤 미복원 — 상위 이동 시 떠나온 폴더에 커서. 뒤로/앞으로의 커서·스크롤 복원은 후속 | 일부 |
| NV-04 | 세션 복원이 SSH 탭을 버리고, 탭 잠금·이름·순서변경·호스트 표시가 없음 — 원격 탭을 alias+경로로 저장·복원, 탭 라벨에 호스트 접두. 탭 잠금·이름·드래그 정렬은 후속 | 일부 |
| NV-05 | 탭 전환 시 목록 갱신·감시자 재설정이 없음 → 낡은 목록 + 자동 새로고침 끊김 — 탭 전환 시 재로드 + pane_watch_set 갱신 | 고침 |
| NV-06 | 타이핑으로 파일명 점프(type-ahead) 없음 — 글자를 쳐도 아무 일도 안 일어난다 — 1초 버퍼 접두사 점프, 같은 글자 반복은 다음 후보로 순환. 이미 매인 키는 커맨드가 우선 | 고침 |
| NV-07 | 자연 정렬 없음(`file10` 이 `file2` 앞), 폴더 우선이 하드코딩 — 자연 정렬 적용. dirs_first 설정화는 후속 | 일부 |
| NV-10 | "커서 항목을 반대 패널에서 열기" / "새 탭에서 열기" 단축키 없음 (+ 경로 결합 규칙 위반) — pane.openInOther (Ctrl+→) / tab.openInNew (Ctrl+↑) + childLocation 사용 | 고침 |
| NV-11 | 원격 탐색 요청 경쟁 조건 — 취소/순서 보장 없음 — 패널별 navigate 토큰 — 늦게 도착한 응답과 그 실패 토스트를 버린다 | 고침 |
| NV-14 | 정렬 키가 DESIGN.md 와 다르고, 번호로 탭 이동하는 커맨드가 없음 — tab.goto1..9 (Alt+1..9). 정렬 키는 맥에서 ⌘⌥1..5 로 분리 | 고침 |
| NV-16 | 빠른 필터가 부분문자열 전용, 필터 중 `..` 가 사라짐, 필터 고정 옵션 없음 — 필터 중에도 `..` 유지는 미적용 — 후속 | 일부 |
| NV-17 | 사이드바/미리보기 열림 상태가 재시작 시 초기화 — 사이드바·미리보기 열림 상태 localStorage 영속 | 고침 |
| NV-18 | 루트에서 ↑ 버튼이 활성처럼 보임, 마지막 탭 Ctrl+W 무반응 — 루트에서 ↑ 비활성(canUp). 마지막 탭 Ctrl+W 는 후속 | 일부 |
...
| OP-06 | 오류 1건이면 전체 중단 — 재시도 / 건너뛰기 / 일시정지 없음 — 취소만 즉시 멈추고 항목 실패는 모아 끝까지 진행 후 한 번에 요약. 일시정지·개별 재시도는 후속 | 고침 |
...
| SS-08 | `~/.ssh/config` 목록이 시작 시 1회 로드 — 새로고침 없음 — 창 포커스 복귀 시 자동 재로드 + Hosts 섹션 ⟳ | 고침 |
| SS-12 | 팔레트/단축키로 ssh-config 호스트에 접속 불가 — Ctrl+K / Ctrl+Shift+K 미구현, 접속이 3단계 — config 호스트를 팔레트에 + connection.quick (Ctrl+Shift+K). 단일 클릭 즉시 접속은 후속 | 일부 |
...
- **심각도**: Med · **레이어**: BE + FE · **고침**: 취소만 즉시 멈추고 항목 실패는 모아 끝까지 진행 후 한 번에 요약. 일시정지·개별 재시도는 후속
...
- **심각도**: High · **레이어**: BE + FE · **고침**: frecency 키를 host_alias 로, 점퍼가 alias 로 (필요시 접속 후) 연다. 구버전 항목은 폐기
...
- **심각도**: Med · **레이어**: FE · ✔ 직접 검증 · **고침**: bookmark.goto1..9 (Alt+Shift+1..9)
...
- **심각도**: Med · **레이어**: FE · **고침**: 추가·중복·실패 토스트 + 접힌 섹션 자동 펼침
...
- **심각도**: High · **레이어**: FE · **고침**: 맥 전용 기본 키(정렬 ⌘⌥1..5, 숨김 ⇧⌘., 미리보기 ⇧⌘P, 경로 ⇧⌘G) + 숫자열을 e.code 로
...
- **심각도**: High · **레이어**: BE + FE · **일부 고침**: 원격 탭을 alias+경로로 저장·복원, 탭 라벨에 호스트 접두. 탭 잠금·이름·드래그 정렬은 후속
...
- **심각도**: Med · **레이어**: FE · **고침**: 1초 버퍼 접두사 점프, 같은 글자 반복은 다음 후보로 순환. 이미 매인 키는 커맨드가 우선
...
- **심각도**: Med · **레이어**: FE · **고침**: pane.openInOther (Ctrl+→) / tab.openInNew (Ctrl+↑) + childLocation 사용
...
- **심각도**: Med · **레이어**: FE (→ BE) · **고침**: 패널별 navigate 토큰 — 늦게 도착한 응답과 그 실패 토스트를 버린다
...
- **심각도**: Med · **레이어**: FE · **고침**: tab.goto1..9 (Alt+1..9). 정렬 키는 맥에서 ⌘⌥1..5 로 분리
...
- **심각도**: Low · **레이어**: FE · **일부 고침**: 필터 중에도 `..` 유지는 미적용 — 후속
...
- **심각도**: Low · **레이어**: FE · **고침**: 사이드바·미리보기 열림 상태 localStorage 영속
...
- **심각도**: Low · **레이어**: FE · **일부 고침**: 루트에서 ↑ 비활성(canUp). 마지막 탭 Ctrl+W 는 후속
...
- **심각도**: High · **레이어**: settings + FE (+BE) · **고침**: size_units / date_format / dirs_first / editor_command + F4(file.edit) 추가
...
- **심각도**: High · **레이어**: BE + FE · **일부 고침**: 크기·기간·종류 필터 + 결과에 크기·수정시각. 정규식·결과에서 바로 작업은 후속
...
- **심각도**: Med · **레이어**: FE · **고침**: 메뉴·툴바·경로바·상단바가 useKeyHint 로 현재 키맵을 표시. i18n 문구에서 키 제거
...
- **심각도**: Med · **레이어**: FE · **고침**: 설정이 꺼져 있으면 메뉴·팔레트에 아예 등록하지 않는다
...
- **심각도**: Med · **레이어**: FE · **일부 고침**: 최근 사용 우선 + 번역·원문·id 매칭. 카테고리 그룹 헤더와 모드 접두는 후속
...
- **심각도**: Med · **레이어**: FE · **일부 고침**: 검색 패널이 formatErr 사용. 원인별 액션 버튼은 후속
...
- **심각도**: Med · **레이어**: FE · **고침**: 창 포커스 복귀 시 자동 재로드 + Hosts 섹션 ⟳
...
- **심각도**: Med · **레이어**: FE · **일부 고침**: config 호스트를 팔레트에 + connection.quick (Ctrl+Shift+K). 단일 클릭 즉시 접속은 후속
