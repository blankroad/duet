...
export function AdHocConnectDialog({
  ...
  const doConnect = async (trust: boolean, replaceChanged = false) => {
    ...
    const savedPasswordAlias =
      !pw && prefill && hasSavedPassword && vault.unlocked
        ? prefill.alias
        : null;
  ...
  return (
    <>
      <DialogShell
        open={open}
        title={t("dialog.adhoc.title")}
        description={t("dialog.adhoc.desc")}
        icon={Server}
        onClose={handleClose}
        footer={
      ...
      >
        ...
        {prefill && vault.exists && !vault.unlocked && (
          <button
            ...
            className="flex w-full items-center justify-center gap-1 rounded border border-border px-2 py-1 text-meta text-fg-muted hover:bg-subtle hover:text-fg"
          >
            <Lock size={11} />
        ...
        {phase.kind === "error" && (
          <div className="rounded-panel border border-danger/30 bg-danger/10 px-3 py-2 text-meta">
