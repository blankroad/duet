...
describe("connections store", () => {
  ...
  it("upsertActive adds and overwrites by id", () => {
    ...
    useConnections
      .getState()
      .upsertActive({ ...mkActive("a:1", "a"), host_ip: "10.0.0.99" });
  ...
  it("setState updates only the matching id; no-op for unknown id", () => {
    useConnections
      .getState()
      .upsertActive(mkActive("a:1", "a", { kind: "connecting" }));
    useConnections.getState().setState("a:1", { kind: "connected" });
    expect(useConnections.getState().active["a:1"]?.state.kind).toBe(
      "connected",
    );
    useConnections
      .getState()
      .setState("missing", { kind: "error", message: "x" });
  ...
  it("stateByAlias picks highest-priority state per alias", () => {
    ...
    useConnections
      .getState()
      .upsertActive(mkActive("a:2", "a", { kind: "connected" }));
    useConnections
      .getState()
      .upsertActive(mkActive("b:1", "b", { kind: "error", message: "auth" }));
  ...
  it("liveByAlias skips dead connections (backend pool 에서 사라진 error entry)", () => {
    ...
    useConnections
      .getState()
      .upsertActive(
        mkActive("a:1", "a", { kind: "error", message: "gave up" }),
      );
