 ...
 */
export function resolveDropAt(x: number, y: number): { pane: PaneId; folder: string | null } | null {
...
/** 좌표 아래 드롭 대상의 최종 Location (폴더면 그 폴더, 아니면 패널 현재 디렉토리). */
export function dropLocationAt(x: number, y: number): Location | null {
  ...
  return d.folder ? childLocation(base, d.folder) : base;
}
